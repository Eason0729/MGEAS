    ...
    xhr.onreadystatechange = function (res) {
      if (this.readyState == 4 && this.status == 200) {
        ...
        document.querySelector(".term-of-use").innerHTML =
          basic_data.term || "<p class='text-break'>copyright wasn't marked, contact the owner for furthur infomation</p>";
...
function start_play() {
  ...
  interval_list = [];
  if (!started_) {
    document.querySelector("title").innerText="MGEAS - "+basic_data.path.audio.split("/").pop();
    audio.muted=false;
  ...
  if (pause) {
    ...
    setTimeout(()=>{
      interval_list.push(setInterval(function () {
        drop_list.forEach((element) => {
          element.y += basic_data.play.drop.speed;
        });
      }, 1000 / 60));
